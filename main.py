    ...
    def a():
        ...
        try:
            ...
            for i in range(3):
                matrix.append([])
                for j in range(3):
                    matrix[-1].append(int(e_matrix[(3*i)+j].get()))
            ...
            e_result.insert(0,message_encrypt)
        except:
            e_result.delete(0,tk.END)
            e_result.insert(0,'ERROR')
    a()
...
def decoded():
    def a():
        ...
        try:
            ...
            e_result.delete(0,tk.END)
            e_result.insert(0,message_decoded)
        except:
            e_result.delete(0,tk.END)
            e_result.insert(0,'ERROR')
    a()
...
e_message=tk.Entry(index)
e_message.grid(row=1, column=3,columnspan=3)

tk.Label(index, text="Matrix ").grid(row=4, column=1)
e_matrix=[]
for i in range(9):
    e_matrix.append(tk.Entry(index, width=4))
    e_matrix[i].grid(row=3+(i//3), column=3+(i%3))


tk.Label(index, text="Result ").grid(row=9, column=1)
e_result=tk.Entry(index)
e_result.grid(row=9, column=3,columnspan=3)


tk.Button(index, text='Coded',command=coded).grid(row=7, column=1)
tk.Button(index, text='Decoded',command=decoded).grid(row=7, column=3,columnspan=3)
